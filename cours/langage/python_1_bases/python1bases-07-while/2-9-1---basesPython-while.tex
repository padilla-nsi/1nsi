...
\setcounter{secnumdepth}{0} % Turns off numbering for sections
    ...
    \geometry{a4paper,tmargin=3cm,bmargin=2cm,lmargin=1cm,rmargin=1cm}\fancyhead[L]{Thème à définir}\fancyhead[C]{\bfseries\MakeUppercase{2-9-1---basesPython-while}}\fancyhead[R]{\monthyeardate\today}
...
\begin{document}
    ...
    \author{Pascal Padilla}\title{2-9-1---basesPython-while}
% \maketitle
...
\section{\texorpdfstring{Boucle
\texttt{while}}{Boucle while}}\label{boucle-while}}

    \hypertarget{principe-de-la-boucle-tant-que}{%
\subsection{\texorpdfstring{1. Principe de la boucle
\texttt{tant\ que}}{1. Principe de la boucle tant que}}\label{principe-de-la-boucle-tant-que}}

    Si l'on ne sait pas à l'avance le nombre de tours de boucle à effectuer,
la boucle \texttt{for} est difficile à manipuler.

Pour cela, nous allons voir dans ce chapitre la boucle \textbf{tant
que}, notée \texttt{while} en Python qui permet de répondre à ce
problème.
...
\tightlist
\item
  d'une part le mécanisme de la boucle \texttt{for} qui permet
  d'effectuer un nombre prédéfini d'itérations d'un bloc d'instructions
\item
  d'autre part le branchement conditionnel \texttt{if} qui permet
  d'effectuer un bloc d'instruction en fonction d'un test effectué sur
  le moment.
\end{itemize}

    Les boucle \emph{tant que}, ou boucle \texttt{while}, combinent ces deux
principes d'une nouvelle manière en conditionnant la poursuite des
itérations à un test effectué préalablement \emph{à chaque tour de
boucle}.

    \hypertarget{uxe9tat-des-variables}{%
\subsubsection{1.1 État des variables}\label{uxe9tat-des-variables}}

    Exécuter le code ci-dessous dans Python Tutor et noter l'état des
variables pour chaque ligne exécutée ainsi que l'éventuel affichage.

\begin{longtable}[]{@{}clll@{}}
\toprule
ligne & \texttt{n} & \texttt{i} & affichage\tabularnewline
\midrule
\endhead
1 & \texttt{11} & &\tabularnewline
2 & \texttt{11} & \texttt{2} &\tabularnewline
3 & & &\tabularnewline
4 & & &\tabularnewline
5 & & &\tabularnewline
3 & & &\tabularnewline
...
\PY{k}{while} \PY{n}{i} \PY{o}{\PYZlt{}}\PY{o}{=} \PY{n}{n}\PY{p}{:}
    ...
    \PY{n}{i} \PY{o}{=} \PY{l+m+mi}{2} \PY{o}{*} \PY{n}{i}
\PY{n+nb}{print}\PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{Fin}\PY{l+s+s2}{\PYZdq{}}\PY{p}{)}
\end{Verbatim}
\end{tcolorbox}
    }

    \hypertarget{lien-avec-la-boucle-for}{%
\subsubsection{\texorpdfstring{1.2 Lien avec la boucle
\texttt{for}}{1.2 Lien avec la boucle for}}\label{lien-avec-la-boucle-for}}

    Transformer la boucle \texttt{for} suivante en boucle \texttt{while} :
...
\subsubsection{1.3 Divergence}\label{divergence}}

    \hypertarget{sortir-dune-boucle}{%
\subsection{2. Sortir d'une boucle}\label{sortir-dune-boucle}}

    Proposer un algorithme pour le problème suivant :

\emph{demander à l'utilisateur de saisir un nombre positif ou nul, et
continuer à lui demander de nouvelles saisies tant que la donnée saisie
n'est pas valide.}

    \hypertarget{chercher-dans-un-tableau}{%
\subsection{3. Chercher dans un
tableau}\label{chercher-dans-un-tableau}}

    Écrire un programme \texttt{appartient} prenant en paramètre un élément
\texttt{elem} et un tableau \texttt{tab}, et renvoyant \texttt{True} si
l'élément \texttt{elem} apparaît au moins une fois dans le tableau
\texttt{tab}, et renvoyant \texttt{False} sinon.

    \hypertarget{terminaison}{%
\subsection{4. Terminaison}\label{terminaison}}
