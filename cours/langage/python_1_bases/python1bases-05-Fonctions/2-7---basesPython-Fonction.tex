...
\setcounter{secnumdepth}{0} % Turns off numbering for sections
    ...
    \geometry{a4paper,tmargin=3cm,bmargin=2cm,lmargin=1cm,rmargin=1cm}\fancyhead[L]{Thème à définir}\fancyhead[L]{\adjustimage{height=1cm, valign=m}{/home/bouscadilla/Documents/Code/nbconvert/template/latex/pdf_solution/papier_eleve_ico_langage}\ttfamily\scshape Langage}\fancyhead[C]{\bfseries\MakeUppercase{2-7---basesPython-Fonction}}\fancyhead[C]{\bfseries\MakeUppercase{2 --- Programmer en Python}}\fancyhead[R]{\monthyeardate\today}
...
\begin{eleve}
    \textbf{2.7-Pythagore} Définir une fonction \texttt{test\_Pythagore} qui
prend trois entiers \(a\), \(b\) et \(c\) en arguments et renvoie un
booléen indiquant si \(a^2 + b^2 = c^2\).
...
argument et renvoie sa valeur absolue.
        ...
        \end{eleve}\begin{eleve}
    Modifier la fonction \texttt{multiples} pour qu'elle prenne la limite en
argument plutôt que d'utiliser la valeur \(999\). En déduire une
fonction \texttt{multiples\_3\_ou\_5(n)} qui renvoie la somme des
multiples de 3 ou 5 inférieurs ou égaux à n.
        
        \end{eleve}\begin{eleve}
    Écrire une fonction max2(a, b) qui renvoie le plus grand des deux
entiers \texttt{a} et \texttt{b}.
...
On suppose que \(k\geq 0\) et on rappelle que \(x^0 = 1\).
        ...
        \end{eleve}\begin{eleve}
    Écrire une fonction \texttt{bisextile(annee)} qui renvoie un booléen
indiquant si l'année \texttt{annee} est une année bissextile.
...
\texttt{bissextile} de l'exercice précédent est disponible et vous
...
La fonction \texttt{bissextile} de l'exercice précédent est disponible
(ce qui est bien pratique).

On suppose que le mois \texttt{mois} est un entier compris entre 1 (pour
...
\texttt{nb\_jours(j\_depart,\ m\_depart,\ a\_depart,\ j\_fin,\ m\_fin,\ a\_fin)}
qui renvoie le nombre de jours compris entre deux dates données.
        
        \end{eleve}\begin{eleve}
    En utilisant la fonction (disponible) \texttt{nb\_jours\_mois}, écrire
un programme qui demande une année à l'utilisateur et affiche le
calendrier de cette année. C'est-à-dire la liste de tous les jours de
tous les mois.
